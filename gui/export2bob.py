...
def generate_config_file(outfile, typ, title, params):
    ...
    for param in params:
        ...
        outfile.write("    <pv_name>$(P){0}_Saved</pv_name>\n".format(param['name']))
        outfile.write("    <x>380</x>\n")
        outfile.write("    <y>{0}</y>\n".format(row*24+2))
        outfile.write("    <width>130</width>\n")
        outfile.write("    <height>20</height>\n")
        outfile.write("  </widget>\n")

#        outfile.write("  <widget type=\"combo\" version=\"2.0.0\">\n")
#        outfile.write("    <items>\n")
#        outfile.write("      <item>Do all</item>\n")
#        if typ.lower() == "aroc":
#            outfile.write("      <item>Do position</item>\n")
#        outfile.write("    </items>\n")
#        outfile.write("    <x>515</x>\n")
#        outfile.write("    <y>{0}</y>\n".format(row*24+2))
