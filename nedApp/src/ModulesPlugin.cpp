...
ModulesPlugin::ModulesPlugin(const char *portName, const char *parentPlugins, const char *pvName, const char *dbPath)
    : BasePlugin(portName, parentPlugins, REASON_OCCDATA, 1, 10, 0, BasePlugin::defaultInterfaceMask | asynOctetMask, BasePlugin::defaultInterruptMask | asynOctetMask)
    , m_outCfg(false)
    , m_disableTimer(true)
    , m_dbPath(dbPath)
{
...
asynStatus ModulesPlugin::writeInt32(asynUser *pasynUser, epicsInt32 value)
{
    if (pasynUser->reason == Discover) {
        ...
        m_outCfg = false; // Work-around to prevent CfgStatus to got to 2=discovered unintentionally
        if (value == 0) {
            if (!readDbFile())
                return asynError;
            ...
            int cfgStatus;
            getIntegerParam(CfgStatus, &cfgStatus);
            if (cfgStatus != 0) {
                if (!writeDbFile())
                    return asynError;
                setIntegerParam(CfgStatus, 0);
...
void ModulesPlugin::processData(const DasPacketList * const packets)
{
    int nDiscovered;
    int nVerified;
...
{
    DasPacket *packet = DasPacket::createOcc(DasPacket::HWID_SELF, moduleId, DasPacket::CMD_DISCOVER, 0, 0, 0);
    if (!packet) {
...
void ModulesPlugin::reqLvdsDiscover(uint32_t moduleId)
{
    DasPacket *packet = DasPacket::createLvds(DasPacket::HWID_SELF, moduleId, DasPacket::CMD_DISCOVER, 0, 0, 0);
    if (!packet) {
        LOG_ERROR("Failed to allocate DISCOVER packet");
        return;
    }
...
void ModulesPlugin::reqVersion(uint32_t moduleId)
{
    ...
    packet = DasPacket::createOcc(DasPacket::HWID_SELF, moduleId, DasPacket::CMD_READ_VERSION, 0, 0, 0);
    if (!packet) {
...
void ModulesPlugin::reqLvdsVersion(uint32_t moduleId)
{
    DasPacket *packet = DasPacket::createLvds(DasPacket::HWID_SELF, moduleId, DasPacket::CMD_READ_VERSION, 0, 0, 0);
    if (!packet) {
        LOG_ERROR("Failed to allocate READ_VERSION packet");
        return;
    }
