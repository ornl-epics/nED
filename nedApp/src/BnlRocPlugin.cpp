...
EPICS_REGISTER_PLUGIN(BnlRocPlugin, 5, "Port name", string, "Dispatcher port name", string, "Hardware ID", string, "Hw & SW version", string, "Blocking", int);

const unsigned BnlRocPlugin::NUM_BNLROCPLUGIN_DYNPARAMS       = 650;  //!< Since supporting multiple versions with different number of PVs, this is just a maximum value
...
BnlRocPlugin::BnlRocPlugin(const char *portName, const char *dispatcherPortName, const char *hardwareId, const char *version, int blocking)
    : BaseModulePlugin(portName, dispatcherPortName, hardwareId, DasPacket::MOD_TYPE_BNLROC, true, blocking,
                       NUM_BNLROCPLUGIN_DYNPARAMS, defaultInterfaceMask, defaultInterruptMask)
    , m_version(version)
{
    if (0) {
...
bool BnlRocPlugin::processResponse(const DasPacket *packet)
{
...
asynStatus BnlRocPlugin::writeOctet(asynUser *pasynUser, const char *value, size_t nChars, size_t *nActual)
{
...
asynStatus BnlRocPlugin::readOctet(asynUser *pasynUser, char *value, size_t nChars, size_t *nActual, int *eomReason)
{
    /* GSG TODO: Anything I need to handle here?
    // Only serving StreamDevice - puts reason as -1
    if (pasynUser->reason == -1) {
...
bool BnlRocPlugin::parseVersionRsp(const DasPacket *packet, BaseModulePlugin::Version &version)
{
...
bool BnlRocPlugin::rspReadConfig(const DasPacket *packet)
{
